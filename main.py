    ...
    for iteration in range(start_iter, min(args.nb_phases + start_iter, int(args.num_classes / args.nb_cl) + 1)):
        ...
        if iteration == start_iter:
            ...
            if not args.continual_norm:
                ...
                ref_model = ResNet(ResidualBlock, [1, 1, 1], input_dim=args.input_dim,
                                   num_classes=tg_model.fc.out_features).to(device)
                ref_model.load_state_dict(tg_model.state_dict())
                saved_bn, l = replace_bn(ref_model, 'model', args.gn_size)
                group_running_mean_list, group_running_var_list, b_size_list = get_continual_variables(tg_model,
                                                                                                       'model')
                load_continual_variables(ref_model, 'model', device, group_running_mean_list, group_running_var_list,
                                         b_size_list)
                ref_model.to(device)
            ...
            if not args.continual_norm:
                ...
                out_features = tg_model.fc.out_features
                if args.cosine_normalization:
                    new_fc = modified_linear.SplitCosineLinear(tg_model.fc.in_features, tg_model.fc.fc1.out_features,
                                                               tg_model.fc.fc2.out_features)
                    new_fc.sigma.data = tg_model.fc.sigma.data
                else:
                    new_fc = modified_linear.SplitLinear(tg_model.fc.in_features, tg_model.fc.fc1.out_features,
                                                         tg_model.fc.fc2.out_features)
