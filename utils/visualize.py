...
def visualize(trans,tg_model,test_loader,testset,device,tb,mode,phases):
    ...
    for lab in range(num_categories):
        ...
        mi = marker_fig[0]
        ax.scatter(tsne_proj[indices,0],tsne_proj[indices,1], c=np.array(cmap(lab)).reshape(1,4), marker = mi,label = str(lab) ,alpha=0.1)
    # for lab in range(num_categories):
    ...
    # ax.legend(loc='center left', bbox_to_anchor=(1, 0.5))
    plt.savefig('/home/s.ayromlou/CVPR19_Incremental_Learning/cifar100-class-incremental/tsne/umap_label_'+tb+'_'+mode+'_'+str(phases)+'.png', bbox_inches='tight')
