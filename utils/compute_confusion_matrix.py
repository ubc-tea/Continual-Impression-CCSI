...
def compute_confusion_matrix(tg_model,evalloader, tg_feature_model = None, class_means = None, print_info=False, device=None):
    if device is None:
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    tg_model.eval()
    if tg_feature_model != None:
    ...
    cm = np.zeros((dim, num_classes, num_classes))
    with torch.no_grad():
