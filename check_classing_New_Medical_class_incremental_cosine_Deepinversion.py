    ...
    for iteration in range(start_iter, min(args.nb_phases+start_iter,int(args.num_classes/args.nb_cl)+1)):
        ...
        if iteration == start_iter:
            ...
            if not args.continual_norm:
                ...
                ref_model = ResNet(ResidualBlock, [1, 1, 1],input_dim=args.input_dim,num_classes=tg_model.fc.out_features).to(device)
                ref_model.load_state_dict(tg_model.state_dict())
                saved_bn,l = replace_bn(ref_model, 'model',args.gn_size)
                group_running_mean_list, group_running_var_list, b_size_list = get_continual_variables(tg_model,'model')
                load_continual_variables(ref_model,'model',device,group_running_mean_list,group_running_var_list,b_size_list)
                ref_model.to(device)
            ...
            if not args.continual_norm:
                ...
                out_features = tg_model.fc.out_features
                if args.cosine_normalization:
                    new_fc = modified_linear.SplitCosineLinear(tg_model.fc.in_features, tg_model.fc.fc1.out_features, tg_model.fc.fc2.out_features)
                    new_fc.sigma.data = tg_model.fc.sigma.data
                else:
                    new_fc = modified_linear.SplitLinear(tg_model.fc.in_features, tg_model.fc.fc1.out_features, tg_model.fc.fc2.out_features)
                new_fc.fc1.weight.data = tg_model.fc.fc1.weight.data
                new_fc.fc2.weight.data= tg_model.fc.fc2.weight.data
                ref_model.fc = new_fc
                ref_model.load_state_dict(tg_model.state_dict())
                saved_bn,l = replace_bn(ref_model, 'model',args.gn_size)
                group_running_mean_list, group_running_var_list, b_size_list = get_continual_variables(tg_model,'model')
        ...
        if iteration == start_iter:
            ...
            if (args.add_data and iteration > args.start_generate_phase) :
                print("Add protoset data with size: ",len(Y_protoset_cumuls),Y_protoset_cumuls)
        ...
        ckp_name = args.main_directory + '/{}_run_{}_iteration_{}_model.pth'.format(main_ckp_prefix, iteration_total, iteration)
        ...
        if args.load_dricet_mode and iteration == args.start_generate_phase:
            ...
            if args.continual_norm:
                load_continual_variables(tg_model,'module',device,
                                         torch_loaded['group_running_mean_list'],
                                         torch_loaded['group_running_var_list'],
                                         torch_loaded['b_size_list'])
                check_training(tg_model,print_var=True)
            model_loaded = True
            print("###############################")
