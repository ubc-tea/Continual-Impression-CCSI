...
class DeepInversionClass(object):
    def __init__(self, bs=84,
        ...
        for module in self.net_teacher.modules():
            if isinstance(module, nn.BatchNorm2d) or isinstance(module, CN4):
    ...
    def get_images(self, net_student=None, targets=None, use_mean_initialization = False, beta_2 = 0.9):
        ...
        if targets is None:
            ...
            targets = torch.LongTensor([random.randint(self.target_classes_min, self.target_classes_max) for _ in range(self.bs)]).to(self.device)
     ...
     , (self.bs, 3, img_original, img_original))).type(torch.FloatTensor).to(self.device)
        elif self.medmnist != 'None':
            inputs_layer = torch.from_numpy(np.random.normal(0, variance
     , (self.bs, 3, img_original, img_original))).type(torch.FloatTensor).to(self.device)
            print("LOOOK AT MEEE")
            # inputs_layer = torch.from_numpy(np.zeros((self.bs, 3, img_original, img_original))).type(torch.FloatTensor).to(self.device)
        else:
            inputs_layer = torch.from_numpy(np.random.normal(0, variance
     , (self.bs, 1, img_original, img_original))).type(torch.FloatTensor).to(self.device)
 #        variance = 1
 #        inputs_layer = torch.from_numpy(np.random.normal(0, variance
 # , (self.bs, 1, img_original, img_original))).type(torch.FloatTensor).to(self.device)
        ...
        if use_mean_initialization:
            for t in range(len(targets)):
                ...
                # print(image_array.shape,np.max(image_array))
                convert_tensor = transforms.ToTensor()
                image_array = convert_tensor(np.divide(((np.array(image)/255.0)-mean),std)).to(self.device)
                # print(image_array.max(),image_array.shape)
                # # print("readed image max and min",
                # #       image_array.max(),
                # #       image_array.min(),
                # #       torch.reshape(image_array[:,:,0],(1,224,224)).shape)
                # inputs_layer[t] = inputs_layer[t]/10 + torch.reshape(image_array[:,:,0],(1,224,224))
                if self.skin:
                    inputs_layer[t] = inputs_layer[t]/10 + torch.reshape(image_array,(3,self.image_resolution,self.image_resolution))
                elif self.medmnist != 'None':
                    ...
                    inputs_layer[t] = inputs_layer[t]/10 + torch.reshape(image_array[:,:,0],(1,self.image_resolution,self.image_resolution))
        ...
        vutils.save_image(inputs_layer,'{}/best_images/output_{:05d}_gpu_{}_first.png'.format(self.prefix,
                                                                                  ...
                                                                                  local_rank),normalize=True, scale_each=True, nrow=int(10))
        ...
        # return inputs.data.clone(),targets
    ...
    def generate_batch(self, net_student=None, targets=None, use_mean_initialization = False, beta_2 = 0.9):
        ...
        images,targets = self.get_images(net_student=net_student, targets=targets, use_mean_initialization = use_mean_initialization, beta_2=beta_2)
