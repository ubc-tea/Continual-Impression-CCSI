...
class Modified_medmnist(PathMNIST):
    ...
    def __init__(self,
        ...
        self.view_classes = [0,1,2,3,4,5,6,7,8]
        self.proto_sets_x = []
...
def get_medmnist_dataset(mode='train',image_size =32):
    download = False
    if mode == 'train':
        dataset = Modified_medmnist(split='train', download=download,as_rgb= True,image_size= image_size)
    elif mode == 'test': 
        dataset = Modified_medmnist(split='test', download=download,as_rgb= True,image_size=image_size)
    return dataset
